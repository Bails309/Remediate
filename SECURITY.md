...
- **Admin-gated configuration, user-gated execution.** All config routes (`GET|POST /api/admin/ai`, `POST /api/admin/ai/test`) require `requireAdmin()`. The chat route (`POST /api/vulnerabilities/chat`) requires an authenticated web-app user, is rate-limited on user identity, and returns `400` when the feature is unconfigured or disabled. The "Ask AI" launcher is not rendered at all until an admin enables it.
