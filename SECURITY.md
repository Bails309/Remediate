...
- **No data exfiltration to the model.** The language model receives only the user's question plus a static description of the allowed query fields (`lib/ai/insights.ts#SYSTEM_PROMPT`). Vulnerability rows, hostnames, CVEs, and remediation text are **never** sent to the provider. The model's sole output is a JSON *query plan*.
- **Schema-validated, allow-list query plans.** The model response is parsed and validated against a strict Zod contract (`lib/ai/query-spec.ts`). Risk, status, and scanner values are enum whitelists; strings are length-bounded; `limit` is capped at 100. Zod **strips any unknown keys** the model emits, so a prompt-injection payload that tries to introduce new fields (e.g. raw SQL, arbitrary column names) is silently discarded rather than executed.
- **Deterministic execution under existing RBAC.** The validated plan is translated into a Prisma `where`/`orderBy` by `buildWhereFromSpec` / `buildOrderBy` — there is **no raw SQL and no string interpolation** anywhere in the path. The result is `AND`-combined with the same group-visibility wall used by `GET /api/vulnerabilities` (`visibilityWhere(isAdmin, memberOf)`), so a caller can never retrieve rows they could not already see on the normal list, regardless of what the model returns.
...
- **Admin-gated configuration, user-gated execution.** All config routes (`GET|POST /api/admin/ai`, `POST /api/admin/ai/test`) require `requireAdmin()`. The query route (`POST /api/vulnerabilities/insights`) requires an authenticated web-app user, is rate-limited on user identity, and returns `400` when the feature is unconfigured or disabled. The "Ask AI" bar is not rendered at all until an admin enables it.
- **Auditability.** Every question is written to the audit log as `ai_insight_query` with the question text and the generated spec, so AI-assisted queries are fully reviewable.
- **Bounded egress & timeouts.** Provider calls use an `AbortController` timeout (`lib/ai/provider.ts`) and non-2xx responses surface as `AiProviderError` rather than leaking provider internals to the client. For fully air-gapped deployments, point `AI_PROVIDER=openai-compatible` at a self-hosted endpoint (e.g. Ollama) so no data leaves your network boundary.
