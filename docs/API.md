...
| `POST` | `/api/vulnerabilities/{id}/plan` | 🔒 | Generates an AI remediation package for **one** finding — plan, change-request draft, rollback, validation checks and evidence pointers — returned as `{ plan, generatedAt }`. No body. Runs a single tool-free completion built from that finding's row only, so no other issue's data can enter the answer. Requires the caller to be the assignee, a leader of the owning group, or an admin (`403` otherwise; auditors are always refused); `404` when the id is unknown or outside the caller's visibility wall. Nothing is stored. Rate-limited; audited as `ai_remediation_plan`. `400` when AI is unconfigured/disabled, `502` on provider error or unusable model output. |
