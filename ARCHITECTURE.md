...
> **Current release**: `v2.19.0` (2026-09-18). See [`CHANGELOG.md`](CHANGELOG.md) for the full version history and [`docs/API.md`](docs/API.md) for the API surface.
...
- **`worker` (BullMQ)**: Background job processor running **two parallel workers**:
    ...
    - **Notification Dispatcher & Dual-Feed Scheduler (v2.19.0)**: Evaluates user subscription preferences and dispatches scheduled email digests based on custom UTC schedules. Supports **Environment Threat Digest** (targeted to estate assets) and **Global Threat Feed**, dispatching **two distinct emails** when both options are active.
...
- **Threat Intelligence Centre (Frontend)**: Real-time vulnerability feed with source-aware external linking, independent feed toggles (**Environment Threat Digest** vs **Global Threat Feed**), and the MITRE ATT&CK **Threat Actors** catalogue (`app/(app)/threat-intelligence/actors/`).
- **Environment Threat Matcher** (`lib/threat-intelligence/environment-matcher.ts`, v2.19.0): In-memory correlation engine. Scans active (`Vulnerability`) and historical (`VulnerabilityHistory`) finding records across Nessus CSV, Pentest PDF, and ACR container images to construct an environment footprint (CVEs, package names, software/product tokens). Matches incoming threats by CVE, structured OSV `affectedPackages`, and word-boundary text regex, attaching attribution badges (e.g. `🎯 Matches container package: runc`). Cached for 5 minutes in memory.
...
- **Unified Risk Schema (Prisma)**: Relational datastore for Nessus, pentest, and ACR findings, plus global threats and user-specific intelligence subscriptions with dual-feed support.
...
- **Vulnerability Export Engine** (`lib/export-csv.ts`, `lib/export-pdf.ts`, `app/api/vulnerabilities/export/route.ts`, v2.18.0 / v2.19.0): High-performance export pipeline supporting CSV, JSON, and PDF generation. Features hierarchical operational sortation (Severity → Host finding density descending → Host name → CVSS descending → Title), zero-blank-page PDF pagination, streaming buffer accumulation, spreadsheet formula sanitization (neutralizing DDE injection triggers), and mandatory server-enforced scoping to the user's group visibility wall (`visibilityWhere(isAdmin, memberOf)`).
...
8. **Notification & Dual-Feed Dispatch (v2.19.0)**: The dispatcher evaluates user subscription preferences. If `environmentDigestEnabled` is active, it runs the environment correlation engine against the deployment footprint; if matching threats exist, it sends an environment-branded digest with match attribution badges. If `globalDigestEnabled` is active, it dispatches the global feed. When both options are selected, **two separate emails** are dispatched. If zero environment threats match, the environment email is silently suppressed.
...
13. **Vulnerability Export (v2.18.0 / v2.19.0)**: An authenticated user triggers an export (`CSV`, `PDF`, or `JSON`) on `/buckets` or `/vulnerabilities`. `GET /api/vulnerabilities/export` runs under `requireAuth()` and resolves the user's group context via `getGroupContext(userId)`. Active findings are retrieved from Prisma with `visibilityWhere(isAdmin, memberOf)` and filtered by outstanding statuses (`Open`, `InProgress`, `InProgressWithCR`, `AwaitingVendor`, `NoFixAvailable`). Records are sorted hierarchically (Severity → Host density descending → Host name → CVSS descending → Title). The payload is serialized on-the-fly (CSV via RFC 4180 with spreadsheet formula injection escaping; PDF via in-process `pdfkit` rendering summary metrics and finding cards with zero trailing blank pages; JSON via structured metadata schema), buffered in memory, and returned with `Content-Disposition: attachment` and `Cache-Control: no-store`.
