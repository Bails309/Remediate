...
- **Export outstanding vulnerabilities in CSV, PDF, and JSON formats.** Security teams can now export actionable findings directly to external suppliers, contractors, and compliance auditors without granting them platform accounts.
  - **Export entry points across `/buckets` and `/vulnerabilities`**:
    - Per-bucket card export dropdown on `/buckets` (scoped to that specific bucket).
    - Global "Export All Buckets" button in the `/buckets` page header (unifies findings across all visible buckets).
    - Toolbar Export dropdown on `/vulnerabilities` (exports the currently active search and filter state).
    - Floating selection action bar Export dropdown on `/vulnerabilities` (exports only the specifically checked rows via the `ids` parameter).
  - [`lib/export-csv.ts`](lib/export-csv.ts) — RFC 4180-compliant CSV generation and structured JSON serialization with metadata (`exportedAt`, `totalCount`, `siteId`). Maps 18 standardized columns (`id`, `title`, `risk`, `cvssScore`, `status`, `bucket`, `host`, `port`, `protocol`, `cve`, `scanner`, `crNumber`, `packageName`, `installedVersion`, `remediation`, `solution`, `firstSeen`, `lastSeen`).
  - [`lib/export-pdf.ts`](lib/export-pdf.ts) — in-process executive and technical PDF generation via `pdfkit`. Features an executive summary, severity distribution metrics breakdown (Critical, High, Medium, Low, Info), and detailed multi-page finding cards with color-coded severity badges, CVSS indicators, host/port details, and complete solution and remediation instructions. Operates completely in-memory using Node stream buffers.
  - [`lib/export-client.ts`](lib/export-client.ts) — client-side download coordinator (`triggerVulnerabilityExport`). Manages asynchronous fetches, converts binary streams into temporary Object URLs, triggers automatic downloads via programmatic `<a>` elements with clean file naming (`remediate-[bucket]-outstanding-vulnerabilities-[date].[ext]`), revokes URLs to prevent browser memory leaks, and delivers toast feedback.
  - [`components/ExportDropdown.tsx`](components/ExportDropdown.tsx) — shared client dropdown with CSV, PDF, and JSON actions and loading spinners.
  - [`app/api/vulnerabilities/export/route.ts`](app/api/vulnerabilities/export/route.ts) — HTTP endpoint (`GET /api/vulnerabilities/export`) supporting `format` (`csv` | `json` | `pdf`), `siteId`, `siteIds`, `status`, `includeAllStatuses`, `risk`, `assigneeId`, `groupIds`, `ids`, and `q`.
  - **Actionable status scoping by default**: Filters to outstanding findings (`Open`, `InProgress`, `InProgressWithCR`, `AwaitingVendor`, `NoFixAvailable`), excluding `Remediated` and `FalsePositive` unless `includeAllStatuses=true` is requested.
- **Comprehensive test coverage (+15 tests across 3 suites)**:
  - [`tests/api/vulnerabilities-export.route.unit.test.ts`](tests/api/vulnerabilities-export.route.unit.test.ts) — 7 unit tests asserting authentication enforcement, default outstanding status filtering, format negotiation (`csv`, `json`, `pdf`), bucket ID scoping, `ids` array filtering, and strict Group Visibility Wall boundary enforcement.
  - [`tests/lib/export-csv.unit.test.ts`](tests/lib/export-csv.unit.test.ts) — 6 unit tests validating RFC 4180 CSV generation, column mapping, JSON serialization, and spreadsheet formula injection sanitization.
  - [`tests/lib/export-pdf.unit.test.ts`](tests/lib/export-pdf.unit.test.ts) — 2 unit tests running in a Node environment (`// @vitest-environment node`), proving in-process PDF compilation, document structure, and executive summary rendering.

### Changed
- **"What's New" card rolled forward to `whats-new-sep-2026-v2180`**, leading with the new Vulnerability Export capabilities ([`components/WhatsNew.tsx`](components/WhatsNew.tsx)). Added `whats-new-sep-2026-v2180` to `VALID_TOURS` enum in [`app/api/tours/complete/route.ts`](app/api/tours/complete/route.ts) so dismissal POST succeeds.
- **TypeScript bumped 5.9.3 → 7.0.2**, the native (Go) compiler. `npx tsc` now runs the 7.0 binary; a full `--noEmit` check of all files passes clean, as does the same check under 6.0. This also absorbs the 6.0 default changes — the existing `tsconfig.json` already sets `target`, `module`, `moduleResolution` and `strict` explicitly and uses none of the constructs 6.0 turned into hard errors (`baseUrl`, `moduleResolution: node`, `target: es5`, `downlevelIteration`).
  - **TypeScript 7.0 ships no compiler API**, which breaks every tool that imports `typescript` programmatically. `typescript-eslint` refuses to load against it outright (`typescript-eslint does not support TS 7.0`, exit code 2 — the whole lint run, not one rule), and Next's build-time type check needs either the API or a `typescript/bin/tsc` CLI. Support is tracked for TS ≥ 7.1 in [typescript-eslint#10940](https://github.com/typescript-eslint/typescript-eslint/issues/10940).
  - The fix is the [transition layout the TypeScript team documents](https://devblogs.microsoft.com/typescript/announcing-typescript-7-0/#running-side-by-side-with-typescript-6.0): `typescript` is aliased to `npm:@typescript/typescript6@6.0.2` (the 6.0 API re-export, whose binary is named `tsc6`) and TypeScript 7 is installed alongside as `@typescript/native`, which supplies the `tsc` binary. `npx tsc` → 7.0.2, `npx tsc6` → 6.0.x, and typescript-eslint keeps the API it needs. Both compilers type-check the project cleanly.
  - [`next.config.ts`](next.config.ts) — `experimental.useTypeScriptCli: false`. Next 16 defaults this to `true` and then shells out to `typescript/bin/tsc`, which does not exist in the 6.0 compatibility package (its binary is `tsc6`); the build failed with a misleading "do not have the required package(s) installed" error and an auto-`npm install` that reported "up to date" because the alias already satisfied the dependency. With the flag off, `next build` type-checks through the 6.0 API and passes.
  - `pentest-backend` was already on `typescript@7.0.2` and is unaffected: it has no ESLint or Next integration, so `tsc -p tsconfig.json` is the only consumer.

### Security
- **Spreadsheet Formula Injection (CWE-1236 / DDE Injection) Defense**: [`lib/export-csv.ts`](lib/export-csv.ts) implements `sanitizeAndEscapeCsvCell`, scanning every exported value. Cells beginning with formula trigger characters (`=`, `+`, `-`, `@`, `\t`, `\r`) are prepended with an ASCII single quote (`'`) and wrapped in double quotes, neutralizing macro execution in Microsoft Excel, LibreOffice Calc, and Google Sheets.
- **Group Visibility Wall Enforcement on Bulk Export**: `GET /api/vulnerabilities/export` applies `visibilityWhere(isAdmin, memberOf)` from `lib/group-rbac.ts`. Non-administrators cannot bypass organizational scoping by manipulating `groupIds`, `siteId`, or `ids` parameters; unauthorized records are pruned at the database query layer.
- **In-Memory Streaming & Memory Isolation**: PDF compilation (`pdfkit`) and CSV generation operate strictly in-memory using Node.js stream buffers, avoiding temporary file writes to the filesystem and eliminating temp-file data leakage.
- **Rate Limiting & Proxy Cache Invalidation**: Export endpoints are rate-limited per user/IP and enforce `Cache-Control: no-store` and `Pragma: no-cache` to ensure sensitive vulnerability data is not stored in web proxy or browser caches.
- **`deepmerge-ts` pinned to `8.0.1` via `overrides`**, clearing [GHSA-ggr8-5vv4-36mx](https://github.com/advisories/GHSA-ggr8-5vv4-36mx) (high — stack exhaustion when merging recursive object graphs, fixed in `8.0.0`). The package is not a direct dependency: it reaches production through `prisma` → `@prisma/config` and through `shepherd.js` (the product tour), so the CI `dependency-audit` job blocked on it with `at-or-above-gate=1`. `npm audit fix` proposed downgrading `prisma` to 6.12.0 and `shepherd.js` to 11.2.0 — both semver-major regressions — so the transitive pin is the correct fix. `8.x` keeps the same `engines` (`node >=16`) and the same export map as `7.x`; `prisma validate` still loads `prisma.config.ts` through `@prisma/config`, and the production build is unchanged.
- **The app image now installs with `npm ci` instead of `npm install`** ([`Dockerfile`](Dockerfile), `deps` / `dev` / `prod-deps` stages). `npm install` is free to resolve versions newer than the lockfile, so the tree that shipped in the image was never guaranteed to be the tree the `dependency-audit` gate approved — an advisory could be cleared in CI and still be present in the artefact, or vice versa. `npm ci` installs the lockfile exactly, which makes the audit result and the scanned image describe the same dependency graph. Verified by building the `prod-deps` stage and reading the installed version out of the image: `deepmerge-ts 8.0.1`.
  - [`pentest-backend/Dockerfile`](pentest-backend/Dockerfile) is deliberately left on `npm install`. Its build stage upgrades npm *before* installing precisely so the resolver regenerates a modern transitive tree — the committed lockfile was pinning old `tar` and `brace-expansion` into the runtime image, which is what that stage exists to avoid. Converting it needs a lockfile refresh first, not a flag change.

### Documentation
- `README.md`, `ARCHITECTURE.md`, `SECURITY.md`, `docs/API.md`, and `components/WhatsNew.tsx` updated with exhaustive technical documentation for the Vulnerability Export Engine (v2.18.0), including architecture diagrams, sequence pipelines, parameter tables, and security controls.

## [2.17.0] - 2026-08-19
### Added
- **AI remediation package per finding.** The **Vulnerability Details** side sheet gains a **Remediation package** section that drafts, in one call, an ordered remediation plan, a structured change-request record (title, type, summary, justification, affected systems, implementation steps, risk if not applied, risk of change, service impact, scheduling notes), a rollback with its abort trigger, post-change validation checks with expected results, and the evidence to capture as proof. Output renders as native UI and can be copied or downloaded as Markdown ([`components/RemediationPlanPanel.tsx`](components/RemediationPlanPanel.tsx)).
  - [`lib/ai/remediation-plan.ts`](lib/ai/remediation-plan.ts) — a single JSON-mode completion with **no tools attached**, so the model has no route to the vulnerability search surface and cannot pull context in from other issues; the prompt is built server-side from the one database row the user opened. The reply is validated against a zod schema with per-field length caps, and long scanner text is clamped before it is sent so a verbose plugin description cannot exhaust the completion budget and truncate the JSON.
  - Grounding rules match the rest of the assistant: unknown values become bracketed placeholders (`<fixed version>`) recorded in an **Assumptions to confirm** block rather than invented CVEs, versions or dates.
  - [`app/api/vulnerabilities/[id]/plan/route.ts`](app/api/vulnerabilities/%5Bid%5D/plan/route.ts) — gated to the people who own the work: the group-visibility wall first (an out-of-scope id returns `404`, not `403`, so existence is not disclosed), then `canEditVulnerability` — assignee, leader of the owning group, or admin. Read-only auditors are refused outright. Rate limited per user and written to the audit log as `ai_remediation_plan`. Nothing is persisted; the plan lives in the side sheet for that session only.
...
- **The AI assistant can be given a name.** Site admins set it under **Settings → AI Insights**; it appears on the launcher, in the chat header, on the per-finding launcher in the Vulnerability Details sheet, and in the assistant's own system prompt so it introduces itself correctly. Blank uses the product default of *Ask AI*.
  - [`prisma/schema.prisma`](prisma/schema.prisma) — nullable `AiConfig.assistantName`, migration [`20260813210000_ai_assistant_name`](prisma/migrations/20260813210000_ai_assistant_name/migration.sql). `NULL` means "use the default", so no backfill is required and an installation that never sets a name behaves exactly as before.
  - [`lib/ai/config.ts`](lib/ai/config.ts) — `DEFAULT_ASSISTANT_NAME`, `MAX_ASSISTANT_NAME_LENGTH` (40) and `normaliseAssistantName()`. Resolution order matches every other AI setting: database row → `AI_ASSISTANT_NAME` environment variable → default.
  - [`components/AiChatPanel.tsx`](components/AiChatPanel.tsx) — new optional `name` prop driving the header, the focused-finding title and both `aria-label`s. It deliberately does **not** import `DEFAULT_ASSISTANT_NAME`, because `lib/ai/config` pulls in Prisma and must stay out of the client bundle; the fallback is a local constant that names its source of truth.
  - `GET /api/vulnerabilities/chat` now returns `{ available, assistantName }`, so the page labels itself from the probe it was already making rather than a second round trip.
- **Test coverage for the three highest-risk untested surfaces introduced in 2.16.0**, +103 unit tests in total:
  - [`tests/api/dashboards.unit.test.ts`](tests/api/dashboards.unit.test.ts) — all eight dashboard routes, which previously had none. Asserts the ownership boundary (a non-owner gets `403` on PATCH/DELETE even when the board is published), that a private board returns `404` rather than `403` so its existence is not disclosed, that the widget allowlist rejects both an unknown key (`.strict()`) and an incoherent `source`/`groupBy` pairing, the 24-widget cap, that layout writes are scoped by `dashboardId`, that a published widget re-executes under the **viewer's** context rather than the author's, that a spec persisted by an older release fails closed with `422`, and that execution errors do not leak Prisma internals.
  - [`tests/lib/report-scheduler.test.ts`](tests/lib/report-scheduler.test.ts) — regression guard for the 2.16.0 fix. Proves the hourly delta sync and the actor sync still run when `getReportConfig()` returns `null` or `enabled: false`, plus the 6-hour startup-sync throttle and fault isolation between the threat and reporting branches of a single tick.
  - [`tests/api/threat-actors.unit.test.ts`](tests/api/threat-actors.unit.test.ts) — auth, facet filtering, limit clamping, count/list filter parity, admin gating and rate limiting on the manual refresh, and `502` on upstream feed failure.
- **Prisma schema-drift gate.** A new `schema-drift` CI job runs `prisma migrate diff --from-migrations --to-schema-datamodel --exit-code` against a shadow database and fails the build if `schema.prisma` has been edited without a matching migration. Previously that drift only surfaced at deploy time, when `migrate deploy` would apply an incomplete set.
- **[`.github/dependabot.yml`](.github/dependabot.yml)** — the repository had alerts enabled but no version-update configuration, so patches were only ever applied reactively. Weekly npm (root and `pentest-backend`), Docker and GitHub Actions updates, grouped to keep review volume sane, with framework majors deliberately excluded.

### Changed
- **The AI launcher moved to a floating button in the bottom-right corner**, the corner the chat window itself opens into, so the control now visually expands into the panel it summons. It replaces the full-width banner that sat above the filter grid on the Vulnerabilities page, returning roughly 90px of vertical space to the finding list on every load. The banner's explanatory copy became the button's tooltip rather than being dropped, the launcher unmounts whenever either chat panel is open so it cannot overlap the window (`z-[90]` beneath the panel's `z-[100]`), and it keeps a focus-visible ring for keyboard users.

### Security
- **Removed the npm CLI from the runtime images**, eliminating a recurring class of container CVEs at the source. npm's *vendored* dependencies (`tar`, `brace-expansion`, `ip-address`, `undici`, `sigstore`, `minimatch`) — not this application's — were the largest single source of findings against the app and worker images, and the previous mitigation was pinning `NPM_VERSION` to whichever release happened to have patched them. That was a treadmill: advisories land against npm's tree regardless of anything in `package.json`, and each fix waited on an npm release. This round it stopped working entirely — npm 12.0.2 ships the *same* `tar ^7.5.19` as the pinned 12.0.1, so no bump could clear the alerts.
  - npm is genuinely unnecessary at runtime: dependencies are installed in the separate `prod-deps` stage, and every runtime entrypoint now invokes `node` or a `node_modules/.bin` binary directly. `scripts/app-entrypoint.sh` runs `next start`; `scripts/worker-entrypoint.sh` runs `tsx` for both the DB optimiser and the worker; [`scripts/migrate.js`](scripts/migrate.js) resolves the `prisma` binary from `node_modules/.bin`, falling back to `npx prisma` for local runs outside a container. `next`, `tsx` and `prisma` are all production dependencies, so their binaries are present.
  - **Side benefit: correct shutdown signalling.** The app previously ran as `npm start` → `next start`, so `SIGTERM` hit the npm wrapper rather than Next. Dropping the wrapper makes the real process the container's main process and lets it terminate cleanly.
  - Verified on the rebuilt images: `npm`/`npx` absent, `/usr/local/lib/node_modules` reduced to `corepack`, none of the four flagged packages present, both containers reaching a healthy state (migrations applied, Next serving, all worker queues connected), and the full 72-test e2e suite green.
- **Two unvalidated inputs reaching a URL and a log sink**, surfaced by the newly-added CodeQL scan. Both are fixed by validating at the boundary rather than sanitising at the sink — the first attempt used a `forLog()` escaper, but CodeQL kept flagging it (its `js/log-injection` sanitiser only recognises specific replace patterns) and, more importantly, escaping treats the symptom while the underlying inputs were simply never constrained.
  - **`azureAccountName` was interpolated straight into the storage endpoint URL.** [`lib/storage.ts`](lib/storage.ts) builds `https://${azureAccountName}.blob.core.windows.net` on *every* storage operation, so a value such as `evil.com#` would have persistently redirected uploads to an attacker-chosen host — a stored SSRF, not merely a log issue. `POST /api/admin/storage` and `POST /api/admin/storage/test` now enforce Azure's actual rule (`^[a-z0-9]{3,24}$`). Site-admin-gated, so exploitation requires an already-privileged account, but the value had no constraint at all.
  - **`uploadId` was passed unvalidated to a `@db.Uuid` lookup.** `UploadHistory.id` is a UUID column, so a malformed value made Postgres *throw* rather than return null — `GET /api/uploads/progress?uploadId=notauuid` returned **500** instead of `400`. Separately, `canAccessUpload()` short-circuits to `true` for admins *without* the existence check, so an arbitrary string also reached a `console.warn`. `GET /api/uploads/progress` now rejects a non-UUID up front, which closes the 500, the log injection and the format-string finding together.
  - The progress-route tests used a placeholder id (`up-1`) that the schema could never hold; they now use a real UUID, with new cases asserting that malformed ids are rejected *before* any lookup or Redis read.
- **Documented `toolkit_admin` as a command-execution role.** CodeQL flagged the tools-config write in the pentest backend as *"network data written to file"*. The finding is accurate and **accepted**: `validateToolsConfig()` enforces structure but deliberately does not allowlist binaries, because curating the tool catalogue is the feature, and the role can already execute offensive tooling by design. [`SECURITY.md`](SECURITY.md) now states plainly that `toolkit_admin` should be granted with the same care as a shell account on that host — a trust boundary that had never been written down.
- **`proxy.ts` — the edge RBAC layer — now has a test suite.** The middleware is the outermost of the three enforcement layers and was the single largest untested file in the codebase, despite being rewritten in 2.16.0 to demote `web_app_admin`. [`tests/lib/proxy.test.ts`](tests/lib/proxy.test.ts) adds 46 assertions covering the full role × path matrix (`/admin` is `site_admin` only; `/uploads`, `/buckets`, `/automation` are workspace-admin; `/tools` is toolkit), anonymous redirection, public-path bypass, the baseline hardening headers, the absence of `X-Frame-Options`, `x-powered-by` stripping, HSTS and `upgrade-insecure-requests` gating on TLS, and the CSP nonce lifecycle (issue, cookie flags, reuse, and the guarantee that `'unsafe-inline'` never joins the nonce in `script-src`).
- **The assistant name is treated as a prompt-injection surface.** It is interpolated into the model's system message so the assistant can introduce itself, which means a multi-line value could otherwise open its own instruction block. `normaliseAssistantName()` collapses all whitespace runs to a single space and caps the value at 40 characters, the field is `site_admin`-only, and the newline collapse has a dedicated regression test.
- **Corrected a false privacy claim in the admin UI.** The AI Insights page still read *"The model only translates a question into a database filter — your vulnerability data is never sent to the provider"*, which described the v2.9.0 query planner. That planner was replaced in v2.12.0 by a tool-using assistant that genuinely does send findings to the configured provider. An administrator could have chosen a provider on the strength of a guarantee the product stopped honouring four minor versions ago. The copy now states plainly that findings the asking user can see are sent to the provider, and points at the data-policy decision that implies.
- **Container images are now scanned.** `npm audit` only sees the JavaScript dependency tree; the base image, its OS packages and the bundled Node binary were never inspected by CI. The `docker-build` job now runs Trivy against both the app and worker runner images (CRITICAL/HIGH, `--ignore-unfixed`) and uploads SARIF to code scanning.
- **CodeQL added** ([`.github/workflows/codeql.yml`](.github/workflows/codeql.yml)) for `javascript-typescript` and `actions`, on push, PR and a weekly schedule. Semgrep matches patterns within a file; CodeQL follows taint across files, which is the class of injection and SSRF issue a pattern rule cannot see.
- **Semgrep was silently skipping the largest files.** The default 5s per-rule budget was exceeded on `vulnerabilities-client.tsx`, and after three timeouts Semgrep abandons the entire file — so that component was never scanned for regex-DoS, SSRF or XSS. Raised to `--timeout 120 --timeout-threshold 0`.
- **`lib/redis.ts`** uses `crypto.randomInt()` rather than `Math.random()` for cluster reconnect jitter. The value was never a security decision, but a CSPRNG is free at this call rate and clears `njsscan.crypto.node_insecure_random_generator` without a suppression comment.
- **Pinned `nanoid` to `3.3.18`** via `overrides` for [GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8) (high). Reachable only transitively through `postcss` under `@tailwindcss/postcss`, so it is build-time only and never served a request.

### Fixed
- **The production build broke on two Dependabot dependency majors.** Both were merged automatically and only surfaced in the Docker build, because `next build` type-checks while a plain `npm ci` does not.
  - **recharts 2 → 3** widened its callback parameter types. `Tooltip.formatter` now receives `ValueType | undefined` and `labelFormatter` a full `ReactNode` union (including `bigint`), so the hand-annotated `(value: number, name: string)` signature in [`MyQueueSeverityChart`](components/analytics/MyQueueSeverityChart.tsx) and the unguarded `new Date(val)` in [`TrendChart`](components/analytics/TrendChart.tsx) no longer compiled. Both now narrow the value rather than asserting a type. Verified by rebuilding the image and running the chart-heavy e2e specs (analytics, command centre, dashboards) — a major charting upgrade can type-check and still render nothing, so a green `tsc` was not treated as sufficient.
  - **BullMQ 5 → 6** removed `paused` from `JobType` entirely; pausing is queue-wide rather than a job state. [`scripts/diagnose-queue.ts`](scripts/diagnose-queue.ts) now reports it via `queue.isPaused()`. No runtime queue code was affected — `lib/queue.ts` and `scripts/worker.ts` never counted that state.
- **CI was testing on Node 20 while the images shipped Node 24.** The `Dockerfile` builds `FROM node:lts-slim` — a floating tag — so the runtime moved to Node 24 when that LTS landed, with no change to this repository and no corresponding change in CI, which still pinned `node-version: '20'`. Every workflow now pins `'24'` to match what actually ships. `DEPLOYMENT.md` corrected (it still claimed Node 20) and now flags the floating tag as a reproducibility risk.
- **All GitHub Actions re-pinned to Node 24 runtimes**, clearing the runner deprecation warning: `actions/checkout` v4.3.1 → v5, `actions/setup-node` v4.4.0 → v5, `actions/upload-artifact` v4.6.2 → v5, `github/codeql-action` v3.29.4 → v4, `aquasecurity/trivy-action` → current master. SHAs resolved from the GitHub API rather than transcribed.
- **The Trivy upload step failed the build whenever a scan produced nothing.** `if: always()` combined with `sarif_file: .` meant a failed scan — most commonly a rate-limited vulnerability-database pull from `ghcr.io` on shared runners — surfaced as the misleading `No SARIF files found to upload in "."`. Each image now uploads its own file guarded by `hashFiles(...) != ''`, and both scans read the database from the `public.ecr.aws` mirror.
- **The two image scans would have overwritten each other's results.** They were uploaded as a single submission with no `category`, so code scanning could not tell the app image's findings from the worker's. Each now uploads under its own category.
- **Schema drift between `schema.prisma` and the migration history, caught by the new drift gate on its first run.** `20260315012500_force_sync_schema` added `REDIS` to the `StorageProvider` enum but never removed `LOCAL` and never moved the `StorageConfig.provider` default off it — the note in that file (*"REDIS was added via db push"*) records exactly how the two diverged. A database built from migrations therefore had a three-value enum defaulting to `LOCAL`, while the Prisma client modelled two values defaulting to `REDIS`. Runtime was unaffected because `getStorageProvider()` treats anything other than `AZURE` as Redis, so a `LOCAL` value already behaved as Redis; the defect was that the schema disagreed with itself, and any tooling trusting the schema (drift checks, generated clients, a future `migrate dev`) was working from a false picture. New migration [`20260813220000_storage_provider_drop_local`](prisma/migrations/20260813220000_storage_provider_drop_local/migration.sql) reconciles them.
  - **The generated SQL was not safe as-issued.** `prisma migrate diff` emits a bare `USING ("provider"::text::"StorageProvider_new")` cast, which aborts on any surviving `LOCAL` row — precisely the rows a long-lived installation is most likely to hold. The migration therefore performs an explicit `UPDATE … SET provider = 'REDIS' WHERE provider::text = 'LOCAL'` first. Because `LOCAL` already resolved to Redis at runtime, the rewrite is a semantic no-op. Verified by replaying the legacy schema with a `LOCAL` row and confirming the migration converts it, drops `LOCAL` from the enum, and sets the default to `REDIS`; and by applying all 36 migrations to an empty database.
- **`GET /api/threat-intelligence/actors?limit=abc` reached Prisma as `take: NaN`.** `parseInt` returns `NaN` for junk and `Math.min`/`Math.max` propagate it, so a malformed query string produced a `500` instead of a clamped result. Now falls back to the default of 100. Found by the new route tests.
- **CI no longer swallows migration failures.** The `unit` job ran `prisma migrate deploy || true`, so a broken migration passed CI and only failed on deploy.

### Documentation
- `README.md`, `ARCHITECTURE.md`, `SECURITY.md`, `docs/API.md`, `DEPLOYMENT.md` and `docs/AZURE_ENV_VARS.md` updated for the assistant name, the relocated launcher, the corrected data-handling statement and the new CI gates. `AI_ASSISTANT_NAME` documented alongside the other declarative AI fallbacks.

### Note
- **Upgrading is a single additive migration and no configuration change.** `20260813210000_ai_assistant_name` adds one nullable column. Installations that never set a name see no behavioural difference beyond the launcher's new position.
