...
### Security
- **Log and format-string injection in two route handlers**, both found by the newly-added CodeQL scan. New [`lib/log-safe.ts`](lib/log-safe.ts) `forLog()` strips control characters, escapes `%`, and caps length; the call sites now keep a **static** format string and pass untrusted values as separate arguments.
  - [`app/api/uploads/progress/route.ts`](app/api/uploads/progress/route.ts) interpolated the caller-supplied `uploadId` directly into the `console.warn` format string. Reachable in practice: `canAccessUpload()` short-circuits to `true` for admins *without* the existence check, so an arbitrary `uploadId` reaches the log. `%s`/`%j` could reorder the logged arguments and a newline could forge a log line.
  - [`app/api/admin/storage/test/route.ts`](app/api/admin/storage/test/route.ts) logged the submitted `accountName` (site-admin only, newline injection).
  - Severity is bounded by both paths requiring an authenticated admin, but these logs sit alongside the audit trail during incident review, which is precisely when forged lines matter.
