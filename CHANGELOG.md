...
- **False-positive Semgrep "hardcoded username" finding** on the AI registry `User-Agent` string ([`lib/ai/registry.ts`](lib/ai/registry.ts)). Annotated the constant with a `nosemgrep` suppression and a comment clarifying it is an HTTP header value, not a credential.
### Added
