...
## [Unreleased]
### Changed
- **TypeScript bumped 5.9.3 → 7.0.2**, the native (Go) compiler. `npx tsc` now runs the 7.0 binary; a full `--noEmit` check of all 2,484 files passes clean, as does the same check under 6.0. This also absorbs the 6.0 default changes — the existing `tsconfig.json` already sets `target`, `module`, `moduleResolution` and `strict` explicitly and uses none of the constructs 6.0 turned into hard errors (`baseUrl`, `moduleResolution: node`, `target: es5`, `downlevelIteration`).
