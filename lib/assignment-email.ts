...
export interface LeaderGroupAssignmentItem extends AssignmentItem {
    assigneeName: string | null;
}

export function renderLeaderGroupDigest(
    leader: { name: string },
    group: { id: string; name: string },
    items: LeaderGroupAssignmentItem[]
) {
    const totalCount = items.length;
    const appUrl = process.env.NEXTAUTH_URL || "http://localhost:3000";
    const unassigned = items.filter((i) => !i.assigneeName);
    ...
    const renderLeaderCard = (item: LeaderGroupAssignmentItem) => {
        const style = riskStyles[item.risk] || riskStyles.None;
        const vulnUrl = `${appUrl}/vulnerabilities?id=${item.id}`;
        ...
        <div style="background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 20px; margin-bottom: 16px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.02);">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div style="font-size: 12px; color: #475569;">Owner: <span style="font-weight: 700; color: ${item.assigneeName ? '#0f172a' : '#ef4444'};">${owner}</span></div>
                <a href="${vulnUrl}" style="color: #2563eb; font-size: 13px; font-weight: 700; text-decoration: none;">View &rarr;</a>
            </div>
        </div>
        `;
    };
    ...
    const contentHtml = `
    <div style="margin-bottom: 32px; padding-bottom: 24px; border-bottom: 1px solid #f1f5f9;">
        <h1 style="color: #0f172a; font-size: 28px; font-weight: 800; margin: 0 0 12px 0; letter-spacing: -0.02em; line-height: 1.2;">${group.name} — Leader Digest</h1>
        <p style="color: #64748b; font-size: 16px; margin: 0; line-height: 1.5;">
            Hello ${leader.name}, your group currently owns <strong>${totalCount}</strong> active item${totalCount === 1 ? "" : "s"}.
            ${unassigned.length > 0 ? `<strong>${unassigned.length}</strong> ${unassigned.length === 1 ? "is" : "are"} not yet assigned to a person.` : "All items have an owner."}
        </p>
    </div>
    ...
    <table border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom: 32px;">
        <tr>
            ...
            <td width="33%" style="padding-left: 8px;">
                <div style="background-color: #f1f5f9; border: 1px solid #e2e8f0; border-radius: 16px; padding: 16px; text-align: center;">
                    <div style="font-size: 10px; font-weight: 800; color: #475569; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 4px;">Unassigned</div>
                    <div style="font-size: 24px; font-weight: 800; color: #1e293b;">${unassigned.length}</div>
                </div>
            </td>
        </tr>
    </table>
    ...
    ${unassigned.length > 0 ? `
    <div style="margin-bottom: 32px;">
        <h2 style="color: #1e293b; font-size: 14px; font-weight: 800; text-transform: uppercase; margin: 0 0 16px 0; letter-spacing: 0.1em;">Needs an owner</h2>
        ${unassigned.map(renderLeaderCard).join("")}
    </div>
    ` : ""}

    ${critical.length > 0 ? `
    <div style="margin-bottom: 32px;">
        <h2 style="color: #ef4444; font-size: 14px; font-weight: 800; text-transform: uppercase; margin: 0 0 16px 0; letter-spacing: 0.1em;">Critical</h2>
        ${critical.map(renderLeaderCard).join("")}
    </div>
    ` : ""}

    ${high.length > 0 ? `
    <div style="margin-bottom: 32px;">
        <h2 style="color: #f97316; font-size: 14px; font-weight: 800; text-transform: uppercase; margin: 0 0 16px 0; letter-spacing: 0.1em;">High</h2>
        ${high.map(renderLeaderCard).join("")}
    </div>
    ` : ""}

    <div style="text-align: center; margin: 48px 0;">
        <a href="${appUrl}/vulnerabilities?groupIds=${group.id}" style="background-color: #0f172a; color: #ffffff; padding: 18px 36px; border-radius: 12px; font-size: 15px; font-weight: 700; text-decoration: none; display: inline-block; letter-spacing: 0.02em; border: 1px solid #1e293b; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1);">
            OPEN GROUP QUEUE
        </a>
    </div>

    <div style="margin-top: 48px; padding-top: 32px; border-top: 1px solid #f1f5f9; font-size: 12px; color: #94a3b8; text-align: center; line-height: 1.8;">
        <p style="margin: 0 0 8px 0;">You receive this digest because you lead the <strong>${group.name}</strong> group.</p>
        <p style="margin: 0; font-style: italic;">Remediate &copy; High-Fidelity Security Orchestration</p>
    </div>
    `;

    return renderEmailLayout({
        title: `${group.name} leader digest`,
        preheader: `${totalCount} active item${totalCount === 1 ? "" : "s"} in ${group.name}.`,
        contentHtml,
    });
}
